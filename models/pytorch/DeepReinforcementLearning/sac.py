...
class SACWrapper(nn.Module):
    ...
    def predict_with_context(self, x_cgm: np.ndarray, x_other: np.ndarray, **context) -> float:
        ...
        **context : dict
        ...
        with torch.no_grad():
            ...
            # Proyectar al espacio de estados para SAC
            state = self.combined_layer(combined)
            
            # Obtener acción determinística del actor SAC
            action = self.sac_agent.policy.get_action(state[0].cpu().numpy(), deterministic=True)
            
            # Convertir acción a dosis usando transformación lineal
            action_tensor = torch.FloatTensor([action]).to(CONST_DEVICE)
            dose = self.action_to_dose(action_tensor.view(1, -1))
    ...
    def forward_with_context(self, x_cgm: torch.Tensor, x_other: torch.Tensor, 
        ...
        # Codificar entradas (asegurar formato correcto para Conv1D)
        ...
        for i in range(batch_size):
            ...
            action = self.sac_agent.policy.get_action(state[i].cpu().numpy(), deterministic=deterministic)
            actions[i] = torch.tensor(action, device=CONST_DEVICE)
        
        # Transformar a dosis apropiadas
        doses = self.action_to_dose(actions)
