...
def process_subject(subject_path, idx):
    ...
    try:
        ...
        print(f"Error renaming columns in {os.path.basename(subject_path)}: {e}")
        return []
    ...
    iob_values = []
    for row in bolus_df.rows(named=True):
        try:
            iob = calculate_iob(row['date'], basal_df)
            iob_values.append(iob)
        except Exception as e:
            print(f"Error calculating IOB: {e}")
            iob_values.append(0.0)
    ...
    for row in tqdm(bolus_df.rows(named=True), total=bolus_df.height, desc=f"Processing {os.path.basename(subject_path)}", leave=False):
        try:
            ...
            if cgm_window is not None:
                ...
                # Add new feature: CGM trend (slope of last 5 points)
                ...
                features = {
                    ...
                    'cgm_trend': cgm_trend  # New feature
                }
                processed_data.append(features)
        except Exception as e:
            print(f"Error processing row in {os.path.basename(subject_path)}: {e}")
            continue

    elapsed_time = time.time() - start_time
    print(f"Processed {os.path.basename(subject_path)} (Subject {idx+1}) in {elapsed_time:.2f} seconds")
...
def preprocess_data(subject_folder):
    ...
    print(f"Total de muestras: {df_processed.height}")
    print("Schema de df_processed:")
    ...
    if missing_columns:
        print("Available columns:", df_processed.columns)
...
def plot_feature_importance(model, X_cgm, X_other, y, scaler_y, feature_names):
    """
    Compute and plot feature importance using permutation importance.
    """
    start_time = time.time()
    ...
    # Create a wrapper class for the PPO model to make it compatible with sklearn
    class PPOWrapper:
    ...
    elapsed_time = time.time() - start_time
    print(f"Feature importance computation complete in {elapsed_time:.2f} seconds")

def count_prediction_types(y_true, y_pred, scaler_y):
    """
    Count predictions in low, medium, and high dose categories.
    """
    y_true_denorm = scaler_y.inverse_transform(y_true.reshape(-1, 1)).flatten()
    y_pred_denorm = y_pred
    
    # Compute thresholds based on true values
    ...
    print(f"Low Dose (< {low_threshold:.2f} units, risk of hyperglycemia): {low_dose} ({low_dose/total*100:.2f}%)")
    print(f"Medium Dose ({low_threshold:.2f} to {high_threshold:.2f} units, normal range): {medium_dose} ({medium_dose/total*100:.2f}%)")
    print(f"High Dose (> {high_threshold:.2f} units, risk of hypoglycemia): {high_dose} ({high_dose/total*100:.2f}%)")
...
def predict_with_ppo(model, X_cgm, X_other):
    predictions = []
    env = InsulinDoseEnv(X_cgm, X_other, np.zeros(len(X_cgm)), scaler_y)
